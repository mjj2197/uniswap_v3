...
export default class OKXConnector extends Connector {
  ...
  constructor({ actions, provider, onError }: EIP1193ConstructorArgs) {
    ...
    this.provider = (window as any).okxwallet as Provider
  ...
  public async activate(): Promise<void> {
    return this.activateAccounts(() => this.provider.request({ method: 'eth_requestAccounts' }).catch(() => this.provider.request({ method: 'eth_accounts' })) as Promise<string[]>)
  }
}
