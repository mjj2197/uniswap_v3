...
import { Percent, SWAP_ROUTER_02_ADDRESSES } from '@jaguarswap/sdk-core'
import { SwapRouter } from '@jaguarswap/router-sdk'
import { FeeOptions, toHex } from '@jaguarswap/v3-sdk'
...
class ModifiedSwapError extends Error {
  constructor() {
    super(t`Your swap was modified through your wallet. If this was a mistake, please cancel immediately or risk losing your funds.`)
  }
}
...
export function useSwapRouterV2Callback(trade: ClassicTrade | undefined, options: SwapOptions) {
  ...
  return useCallback(
    (): Promise<{ type: TradeFillType.Classic; response: TransactionResponse; deadline?: BigNumber }> =>
      trace({ name: 'Swap (Classic)', op: 'swap.classic' }, async (trace) => {
        try {
          ...
          const gasEstimate = await trace.child({ name: 'Estimate gas', op: 'wallet.estimate_gas' }, async (gasTrace) => {
            try {
          ...
          const response = await trace.child({ name: 'Send transaction', op: 'wallet.send_transaction' }, async (walletTrace) => {
            try {
    ...
    [account, chainId, provider, trade, getDeadline, options.slippageTolerance, options.feeOptions, analyticsContext, blockNumber, isAutoSlippage]
  )
}
